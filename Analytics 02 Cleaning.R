...
Analytics <- MergedAnalyticsFile[
	MergedAnalyticsFile$V5 != 'Error' &
	MergedAnalyticsFile$V5 != 'Stored Error' &
	MergedAnalyticsFile$TempDateCheck == '1-1-1 1:1:1' &
	MergedAnalyticsFile$TempNumberIDs == '1',
]

# Naming the columns in Analytics
colnames(Analytics) <- c(
	"Date",
	"UniqueID",
	"UserIDMaybe",
	"Lesson",
...
# TempAnalytics <- Analytics[
#	Analytics$Action == 'Voltage Graph' |
#	Analytics$Action == 'poll' |
#	Analytics$Action == 'poll responses' |
#	Analytics$Action == 'message' |
#	Analytics$LabelFix == 'FromXToY',
# ]
...
Analytics$InstantaneousScreenHeight[grep("Resize", Analytics$Action)] <-
Analytics$InstantaneousScreenHeight[grep("Resize", Analytics$Action)] - 75
...
# TroubleshootingScreenSizes <- subset(Analytics, select = c(
#	UserIDMaybe,
#	Date,
#	Action,
#	Label,
#	InstantaneousScreenWidth,
#	InstantaneousScreenHeight,
#	ScreenSizeRaw)
# )
...
# Populating a PixelRatio variable in case that’s useful in the future.
	...
	Analytics$PixelRatio[grep("Start", Analytics$UIThing)] <-
...
########## Analytics inspection commands: #############
	...
	# table(Analytics$UserLessonEnvironment)
