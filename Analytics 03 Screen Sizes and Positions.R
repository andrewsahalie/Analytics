...
#Creating a table (“LocationDataForTableau”). This involves creating two tables and combining them to duplicate the number of records - one record for each “to” move and one record for each “from” move. This is how Tableau needs it.
...
LocationDataForTableau$UserLessonEnvironment <- paste(LocationDataForTableau$UserIDMaybe, LocationDataForTableau$Lesson, LocationDataForTableau$Environment, sep = "-")
...
# IDForScreenSize is an intermediate variable used to determine when users’ display size has actually changed. Later, that info will be used to create a record of each display size that each user experienced.
...
# Find the Tableau file in Documents -> Analytics Testing, and then just overwrite the data and refresh in Tableau. But it’s easy to recreate if you don’t do that.
